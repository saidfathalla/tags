...
Supports automated format detection and precise semantic metric profiling.
"""
...
try:
    ...
    print("        Please run: pip install rdflib matplotlib numpy\n")
    sys.exit(1)
...
def analyze_knowledge_graph(ttl_file, file_format=None):
    """
    Parses an RDF graph via RDFLib to calculate semantic and topological analytics.
    Exposes structural counts for Data Properties, Object Properties, and Schema Classes.
    """
    # Start with a completely blank slate (no auto-injected default namespaces)
    g = rdflib.Graph(bind_namespaces="none")
    try:
        if file_format:
            g.parse(ttl_file, format=file_format)
        else:
            # Let rdflib automatically deduce serialization format based on file extensions
            g.parse(ttl_file)
    ...
    # --- Extract Input File System Metadata & Scale Metrics ---
    try:
        file_bytes = os.path.getsize(ttl_file)
        if file_bytes < 1024:
            file_size_str = f"{file_bytes} Bytes"
    ...
    if M1 < 1000:
        scale_class = "Micro-Scale Graph (<1K triples)"
    elif M1 < 100000:
        scale_class = "Medium-Scale Schema/Dataset (<100K triples)"
    else:
        scale_class = "Large Enterprise Knowledge Graph (>100K triples)"
    ...
    # Typing variables for distinct property tracking
    ...
    for s, p, o in g:
        ...
        all_graph_nodes.add(str(s))
        if isinstance(o, (URIRef, rdflib.BNode)):
            all_graph_nodes.add(str(o))
        
        # Segment properties based on W3C semantics patterns
        if p == RDF.type:
            taxonomic_triples += 1
            classes[o] += 1
        else:
            # Check if the property acts as a Data Property (Literal values)
            # OR if the predicate itself contains common data property keywords like 'name'
            if isinstance(o, rdflib.Literal) or "name" in str(p).lower():
    ...
    M10 = max_degree / M5 if M5 > 0 else 1.0
    ...
    for node in all_nodes:
        if node not in visited:
            ...
            while queue:
                curr = queue.pop(0)
                comp.append(curr)
                for nbr in list(adj.get(curr, set())):
    ...
    M12 = len([n for n in all_graph_nodes if n not in structural_nodes])
    M13 = sum(1 for comp in components if len(comp) <= 2)

    total_instance_triples = M1 - M4
    M15_ratio = len(instance_predicates) / total_instance_triples if total_instance_triples > 0 else 0.0

    sorted_hubs = sorted(adj.items(), key=lambda x: len(x[1]), reverse=True)[:5]

    return {
        "raw_counts": {"triples": M1, "subjects": M2, "properties": len(predicates), "objects": len(objects), "classes": len(classes)},
        ...
        "metrics": {
            ...
            "M9": [M9, "Average Clustering Coefficient", "Mathematical mean of localized operational clustering patterns."],
            "M10": [M10, "Degree Centrality Heterogeneity", "Exposes whether critical hub nodes dominate network routing. Range [1, inf)."],
            "M11": [M11, "Total Disconnected Components", "Isolated sub-mesh cluster partition groupings."],
            "M12": [M12, "Type-Only Orphan Count", "Nodes possessing classification declarations but lacking structural instance properties."],
            ...
            "M15": [M15_ratio, "Dynamic Predicate Diversity Factor", "Ratio of unique non-taxonomic instance properties to total instance edges. Bounded in (0, 1]."],
...
def export_void_statistics(results, base_name, output_dir):
    """Generates a machine-readable W3C VoID metadata profile."""
    void_g = rdflib.Graph()
...
def display_terminal_dashboard(results, base_name):
    """Outputs visual dashboard grid structure along with structural file headers."""
    title_text = f" FAIR STRUCTURAL METRICS ENGINE PROFILE REPORT: {base_name} "
    border_width = max(len(title_text), 67)
    
    print("\n╔" + "═" * border_width + "╗")
    print("║" + title_text.ljust(border_width) + "║")
    print("╚" + "═" * border_width + "╝")
    
    # Render Physical Asset Telemetry Summary
    print(f" - Physical File Size : {results.get('file_size', 'N/A')}")
    print(f" - Volume Scale Class : {results.get('scale_class', 'N/A')}")
    print(f" ️- Unique Schema Classes Used    : {results.get('class_count', 0)}")
    print(f" - Unique Object Properties Used : {results.get('object_property_count', 0)}")
    print(f" - Unique Data Properties Used   : {results.get('data_property_count', 0)}")
...
def generate_html_dashboard(results, base_name, output_dir, p1_b64, p2_b64, void_path):
    """Assembles analytical web page dashboard."""
    ...
    html_content = f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>FAIR Graph Dashboard</title><style>body {{ font-family: -apple-system, sans-serif; color: #1e293b; background: #f8fafc; padding: 25px; }} .container {{ max-width: 1150px; margin: 0 auto; background: white; padding: 30px; border-radius: 8px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); }} h1 {{ color: #0f172a; margin-bottom: 5px; }} table {{ width: 100%; border-collapse: collapse; margin: 20px 0; }} th, td {{ padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: left; font-size: 14px; }} th {{ background: #f1f5f9; color: #475569; }} .code {{ color: #0f766e; font-family: monospace; }} .value {{ text-align: right; font-weight: bold; font-family: monospace; }} .grid {{ display: flex; gap: 25px; margin-top: 25px; }} .card {{ flex: 1; border: 1px solid #e2e8f0; padding: 20px; border-radius: 6px; text-align: center; background: #fff; }} .card img {{ max-width: 100%; height: auto; }} .hub-list {{ background: #fafafa; padding: 15px; border: 1px solid #e2e8f0; border-radius: 6px; }} .hub-item {{ display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px dashed #e2e8f0; font-family: monospace; font-size: 13px; }} .hub-rank {{ color: #0ea5e9; font-weight: bold; }}</style></head><body><div class="container"><h1>Knowledge Graph FAIR Topological Profile</h1><p>Source Asset: <b>{base_name}</b></p><table><thead><tr><th>Code</th><th>Indicator Description</th><th style="text-align:right;">Measured Value</th><th>Theoretical Context</th></tr></thead><tbody>{rows_html}</tbody></table><h2>Topological Hubs Discovered</h2><div class="hub-list">{hubs_html}</div><h2>Advanced Profiling Visual Frameworks</h2><div class="grid"><div class="card"><h3>Dynamic Predicate Co-occurrence Matrix</h3>{f'<img src="data:image/png;base64,{p1_b64}">' if p1_b64 else '<div>Plot generation unconfigured.</div>'}</div><div class="card"><h3>Taxonomic Class Density Profile</h3>{f'<img src="data:image/png;base64,{p2_b64}">' if p2_b64 else '<div>Plot generation unconfigured.</div>'}</div></div></div></body></html>"""
...
def main():
    ...
    parser.add_argument("ttl_file", help="Path to targeted RDF graph file.")
    parser.add_argument("-o", "--output-dir", default=None, help="Target output directory folder path.")
    parser.add_argument("-f", "--format", default=None, help="RDF serialization format specification (e.g., turtle, xml, json-ld, nt). Auto-guessed if omitted.")
    ...
    base_name = os.path.basename(args.ttl_file) 
    
    print(f"[System Execution] Commencing programmatic parsing of structural schema graph...")
    results = analyze_knowledge_graph(args.ttl_file, file_format=args.format)
    if not results:
        return
